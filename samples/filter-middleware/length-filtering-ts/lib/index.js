...
const lengthPredicate = (context) => {
    return (context.request.type === "message" && context.request.text && context.request.text.length > 10);
};
const lengthValidator = (context) => {
    if (context.request.text && context.request.text.length > 5) {
        return true;
    }
    else {
        return false;
...
const bot = new botbuilder_1.Bot(connector)
    .use(botbuilder_filter_middleware_1.filterMiddleware(lengthValidator, new TestMiddleware_1.TestMiddleware(), new TestMiddleware2_1.TestMiddleware2()))
    .onReceive(context => {
