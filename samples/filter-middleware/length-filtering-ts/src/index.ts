...
const lengthValidator = (context: BotContext): boolean => {
...
const bot = new Bot(connector)
    .use(filterMiddleware(lengthValidator, new TestMiddleware(), new TestMiddleware2()))
    .onReceive(context => {
        if (context.request.type === "message") {
            context.reply(context.request.text as string);
        }
    });